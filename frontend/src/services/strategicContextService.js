...
const StrategicContextService = {
  ...
  /** POST /api/strategic-contexts/{id}/agents — présence explicite d’un agent dans le contexte (OpenSpace). */
  async addContextAgent(contextId, payload) {
    const id = encodeURIComponent(String(contextId || '').trim());
    return apiFetch(`/api/strategic-contexts/${id}/agents`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },

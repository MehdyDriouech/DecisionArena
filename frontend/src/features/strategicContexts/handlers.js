...
function registerStrategicContextsHandlers() {
  ...
  registerAction('open-expert-deep-compare-from-basic', () => {
    ...
    if (!window.DecisionArena.store.canShowExperimentalFeatures?.()) {
      state.toast = window.i18n?.t('contexts.basicCompare.needExperimental')
        ?? 'Activez les fonctionnalités expérimentales (Administration) puis passez en comparaison experte.';
      window.DecisionArena.render?.();
      return;
    }

...
final class StrategicContextController
{
    ...
    public function __construct()
    {
        ...
        $this->contextAgents = new StrategicContextAgentsRepository();
    ...
    public function memoryOverview(Request $req): array
    {
        ...
        foreach ($catalog as $row) {
            ...
            } elseif (!empty($row['needs_memory_sync'])) {
                $diagnostics[] = [
                    'severity' => 'warning',
                    'code' => 'PARTICIPANT_AGENT_MEMORY_NOT_SYNCED',
                    'message' => 'Fichier memory.md présent mais sans trace participant_context_sync (template ou lecture seule sans sync).',
                    'agent_id' => $aid,
                    'recommended_action' => 'contexts.memoryDiagnostics.recommended.syncAgentMemories',
                ];
            }
    ...
    /**
     * POST /api/strategic-contexts/{id}/agents — ajoute explicitement un agent au contexte (OpenSpace / UI).
     *
     * @return array<string,mixed>
     */
    public function addContextAgent(Request $req): array
    {
        $id = strtolower(trim((string)$req->param('id')));
        if (!$this->isValidContextId($id)) {
            return Response::error('Invalid context id', 400);
        }
        $ctx = $this->repo->find($id);
        if ($ctx === null) {
            return Response::error('Context not found', 404);
        }
        $body = $req->body();
        ...
        $agentId = strtolower(trim((string)($body['agent_id'] ?? '')));
        if ($agentId === '' || !$this->agentMemory->isValidAgentId($agentId)) {
            return Response::error('Invalid agent_id', 400);
        }
        ...
        $persona = $this->personas->findById($agentId);
        if ($persona === null) {
            return Response::error('Persona not found', 404);
        }
        if (isset($persona['enabled']) && $persona['enabled'] === false) {
            return Response::error('Persona disabled', 404);
        }

        $inserted = $this->contextAgents->insert($id, $agentId, $source);
        $this->agentMemory->ensureFile($id, $agentId);
        $this->agentMemory->normalizeSoft($id, $agentId);

        if ($inserted) {
            $title = (string)($ctx['title'] ?? $id);
            $date = gmdate('Y-m-d');
            $note = '[' . $date . '] Agent manually added to strategic context `' . str_replace(["\r", "\n", '`'], ' ', $title) . '`. Source: manual_context_agent_add';
            $this->agentMemory->appendRecentNote($id, $agentId, $note, null);
        }
        ...
        return [
            ...
            'source' => $source,
            'inserted' => $inserted,
            'agents_for_open_space' => $this->workspaceAgents->buildForOpenSpaceAgentChat($id),
        ];
    }

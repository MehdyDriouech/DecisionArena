            ...
            foreach ($debateAgents as $agentId) {
                ...
                try {
                    ...
                    $routed  = $this->providerRouter->chat(
                        ...
                        RunTimeoutPolicy::routerOptionsForTelemetry($sessionId, 'jury', $phase, $agentId, $round, null)
                    );
            ...
            foreach (array_slice($minorityAgentIds, 0, 2) as $minorityAgentId) {
                ...
                try {
                    ...
                    $routed  = $this->providerRouter->chat(
                        ...
                        RunTimeoutPolicy::routerOptionsForTelemetry($sessionId, 'jury', 'jury-minority-report', $minorityAgentId, $minorityRound, null)
                    );
                    $content = $routed['content'];
        ...
        if ($synthAgent) {
            ...
            try {
                ...
                $routed  = $this->providerRouter->chat(
                    ...
                    RunTimeoutPolicy::routerOptionsForTelemetry($sessionId, 'jury', 'jury-verdict', 'synthesizer', $verdictRound, null)
                );
                $content = $routed['content'];
                ...
                $this->reportRuntimeEvent(
                    ...
                    [
                        'level' => 'info',
                        'phase' => 'final_synthesis_completed',
                        'round' => $verdictRound,
                        'agent_id' => 'synthesizer',
                        'label' => 'Reponse modele synthese recue',
                    ],
                    [
                        'current_round' => $verdictRound,
                        'total_rounds' => $rounds,
                        'current_phase' => 'final_synthesis_completed',
                        'current_phase_label' => 'Synthese modele terminee',
                        'current_agent_id' => 'synthesizer',
            ...
            foreach ($debateAgents as $agentId) {
                ...
                try {
                    ...
                    $routed = $this->providerRouter->chat(
                        ...
                        null,
                        $this->resolveAgentOverride($agentProviders, (string)$agentId),
                        RunTimeoutPolicy::routerOptionsForTelemetry($sessionId, 'jury', 'retry-round', $agentId, $verdictRound + 1, null)
                    );
        ...
        foreach (array_slice($debateAgents, 0, 3) as $agentId) {
            ...
            try {
                ...
                $routed  = $this->providerRouter->chat(
                    ...
                    null,
                    $this->resolveAgentOverride($agentProviders, (string)$agentId),
                    RunTimeoutPolicy::routerOptionsForTelemetry($sessionId, 'jury', 'jury-mini-challenge', $agentId, $miniChallengeRound, null)
                );
                $content = $routed['content'];
